...
class LLM:
    ...
    def _mock(self, prompt: str, max_tokens: int) -> LLMResult:
        ...
        if "json array" in pl or "array only" in pl:
            # S1 batch topic discovery asks for a JSON array of topic
            # candidate objects.
            if "company_name" in pl and "hq_country" in pl:
                topics = []
                for i in range(3):
                    topics.append({
                        ...
                        "conflict": "An incumbent giant and a market that was not ready",
                ...
                return LLMResult(text=json.dumps(topics),
