...
- **Phase 3: Shorts** — `pipeline.shorts.generate_teaser_script()` asks the writer LLM for separate compressed viral teasers from the full script and beat table (`prompts/shorts_teaser_script.txt`). S13 renders separate standalone TTS tracks using Kokoro speed directly by default, then builds 2 vertical Shorts from reused beat images with rapid cuts, fast Ken Burns motion, slide-left xfade transitions, and no music/SFX. English captions are chunked into short readable cues, optionally hard-burned as Pillow overlays, and saved as `.srt` for upload packaging. Output: `05_video/shorts/short_NN.mp4` + `short_NN.srt` + `teaser_script_NN.txt` + `manifest.json`.
