...
- **Phase 3: Shorts** — `pipeline.shorts.pick_windows()` asks the writer LLM for 3 dramatic 30-second windows (`prompts/shorts_select.txt`). When `cfg.packaging.shorts_exclude_callout_beats: true`, beats with Pillow callouts are removed from candidate selection and overlapping windows are rejected. Each window is cut from `05_video/final.mp4` as 1080×1920 vertical via ffmpeg, with hard-burned subtitles from whisper.cpp (Q-D1: hard subtitles configurable via `cfg.packaging.shorts_burn_subtitles`). Whisper is for captions only; source audio is preserved directly from `final.mp4`. Output: `05_video/shorts/short_NN.mp4` + `manifest.json`.
...
*This file last updated: 2026-06-02 — relocation-safe Shorts packaging.*
