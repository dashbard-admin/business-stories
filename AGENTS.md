...
S06 callout repair is decimal-safe: sentence scanning must not split `$4.5 million` / `$2.6 million` into `$4` / `$2` callouts. The story-quality gate also flags loaded or unsupported regulatory phrasing such as narrator-declared criminality, `refused`, `real profit engine`, `smaller entity`, or `fees are paid correctly` unless the ledger supports or frames it.
