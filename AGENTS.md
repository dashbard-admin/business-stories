...
**Callout repair *(Batch N.5 2026-05-31)*:** after generation, length repair, beat normalization, and forbidden-phrase substitutions, S06 counts `[CALLOUT: ...]` markers. If the writer under-delivers, `_ensure_callout_markers()` scans beat sentences for ledger-safe numeric/date patterns already present in the prose and inserts markers until `callouts.target_total` (default 5) or `callouts.max_total` (default 6). This keeps S08/S12 from shipping a full video with only one overlay when the act-by-act writer obeyed the prose brief but forgot the marker syntax.
