...
- `_apply_visual_continuity_plan()` *(Batch M 2026-05-30)* groups beats into deterministic scenes (`SCENE_NN`) and assigns a `story_world` (`legal_fraud`, `product_platform`, `name_confusion_bridge`, `business_context`, `closing_resolution`). It rewrites each FLUX fallback as a movie-shot prompt with narrative purpose, recurring props, scene title, and beat-specific `must_not_show` constraints. Output includes top-level `visual_continuity_plan` plus per-beat `scene_id`, `scene_title`, `story_world`, `recurring_props`, `visual_must_not`, `movie_shot_prompt`, and `visual_prompt_version`.
