...
Whisper.cpp wrapper for Shorts captions and long-form S12 alignment. `transcribe(wav_path)` returns timed `Segment(start_seconds, end_seconds, text)` rows. S12 uses these rows, when available, to build ASR-aligned long-form SRT/VTT captions and to place Pillow callouts near the actual spoken sentence instead of the old word-ratio estimate. The normalized long-form cache is `04_audio/voice_asr_segments.json`; it is rebuilt when `voice_full.wav` changes. If `whisper-cli` is unavailable, callers fall back gracefully to estimated timing. Configurable via `cfg.asr`, including `asr.long_form_enabled`.
...
**Blueprint + act-by-act generation *(Batch N 2026-05-31; retry budget wired 2026-06-06; smaller staged budgets 2026-06-06)*:** when `production.script_act_by_act_enabled: true`, S06 no longer asks the writer for one huge script first. It runs `script_blueprint.txt` to assign act budgets, fact IDs, hook turns, beat goals, recurring props, and callout opportunities, writes `02_script/script_blueprint.json`, then calls `script_act_generate.txt` once per act with the relevant act blueprint, previous-act tail, narrator persona, fact ledger, and beat range. The staged path now uses `production.max_script_generation_attempts`, adding stronger length directives between attempts and keeping the closest clean draft if all attempts miss. Act budgets are intentionally scaled by `production.script_staged_word_budget_multiplier` (default `0.82`) because the act writer naturally overshoots; for a 1600-word target it feeds ~1312 words of act ceilings so the final draft lands inside the 1500-1800 gate. The output is renumbered, cleaned, repaired for near-miss length issues, and only falls back to the older full-script retry loop if staged generation errors on every attempt or returns no draft. Preview mode still uses the older compact full-script path.
