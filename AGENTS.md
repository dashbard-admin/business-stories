...
**Narrative spine + act-by-act generation *(Batch N 2026-05-31; retry budget wired 2026-06-06; smaller staged budgets 2026-06-06; final beat-id hardening 2026-06-07; story-spine/fact-budget pass 2026-06-08; ending/quality guards 2026-06-08)*:** when `production.script_act_by_act_enabled: true`, S06 first runs `script_narrative_spine.txt` to lock the central question, timeline mode, protagonist desire, fatal misread, first big bet, hidden weakness, darkest moment, mid-roll tension, final image, `ending_mode`, `viewer_question`, and core/support/callout fact IDs. It writes `02_script/script_narrative_spine.json`, then filters the fact ledger down to the selected story facts before blueprint/prose generation. `script_blueprint.txt` then assigns act budgets, causal turns, fact IDs, hook turns, beat goals, recurring props, and callout opportunities, writing `02_script/script_blueprint.json`. `script_act_generate.txt` writes one act at a time with the relevant blueprint slice, previous-act tail, narrator persona, filtered fact ledger, and beat range. The staged path uses `production.max_script_generation_attempts`, adding stronger length directives between attempts and keeping the closest clean draft if all attempts miss. Act budgets are intentionally scaled by `production.script_staged_word_budget_multiplier` (default `0.82`) because the act writer naturally overshoots; for a 1600-word target it feeds ~1312 words of act ceilings so the final draft lands inside the 1500-1800 gate. The output is renumbered, cleaned, repaired for near-miss length issues, and only falls back to the compact `script_generate.txt` full-script retry loop if staged generation errors on every attempt or returns no draft. After every late mutation, including substitutions, short-beat merge, and callout repair, S06 renumbers beats again, removes adjacent/nearby duplicate beat bodies, writes `02_script/script_story_quality_flags.json`, and recomputes `script_meta.json` so S08 and S10 cannot disagree on beat IDs. Preview mode still uses the compact full-script path.
...
| `script_blueprint.txt` *(added Batch N 2026-05-31; causal spine update 2026-06-08)* | S6 primary | `{incident_name}, {narrative_spine_json}, {fact_ledger_json}, {act_specs_json}, …` | Builds a validated causal act/beat/fact blueprint before prose generation. |
