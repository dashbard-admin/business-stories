...
flowchart TB
    ...
    subgraph ADAPTERS [pipeline/  — adapter layer]
        ...
        TTS[tts.py<br/>Chatterbox / Kokoro]
    ...
    %% ============== External services ==============
    LLMGW[(LLM gateway<br/>10.0.4.250:9000<br/>Qwen/Gemma/Chatterbox/Kokoro/VLM)]
...
│   ├── lexicon/
│   │   └── pronunciation_overrides.yaml   ← TTS pronunciation map
│   ├── lint/
...
*(Batch G/H/N.7)* `story_kind` gates narrators, archetypes, and visual styles via optional `suits_story_kinds` fields in `config.yaml`. Entries without that field are universal. The eligible pool is filtered BEFORE cooldown, so e.g. Felix (N5) avoids `underdog_comeback`, and V3 (Sunlit Editorial Comic) is reserved for optimistic `origin`, `disruption`, `pivot`, and `underdog_comeback` topics. Pins on the episode record (set via `--narrator`, `--archetype`, or `--visual-style`) override the cooldown engine's choice entirely.
...
### 5.6 `tts.py`
TTS backend dispatcher. Chatterbox is the production default and hits the oMLX `/v1/audio/speech` endpoint with the `chatterbox-turbo-4bit` model. `make_tts(narrator_id)` can also return Kokoro (`cfg.tts.backend == "kokoro"`) or ElevenLabs (`"elevenlabs"`). S10 chunks the script into beat-sized segments and concatenates with a brief silence between. Backend switch is one config-line flip with graceful fall-back to Kokoro on adapter init failure. Chatterbox `voice_ref_map` currently pins N5→`assets/voice_samples/male1.wav`, N6→`male2.wav`, N7→`female1.wav`, and N8→`female2.wav`. oMLX requires `ref_text` whenever `ref_audio` is sent; exact per-sample `voice_ref_map.N*.ref_text` wins, then global `tts.chatterbox.ref_text`, then `tts.chatterbox.ref_text_fallback`.

### 5.6b `tools/test_chatterbox_tts.py` *(added 2026-06-04, test-only)*
Experimental Chatterbox sampler for the oMLX `/v1/audio/speech` route. Defaults to `chatterbox-turbo-4bit` on `10.0.4.250:9000`, and lets the operator test `voice`, `instructions`, optional `--ref-audio`/`--ref-text`, sampling knobs, chunk size, and one or more exact `--target-wpm` stretched variants. `--ref-audio` accepts a local file and base64-encodes it for oMLX's `ref_audio` field. If `--text` is omitted and `--ref-text` is present, the tool synthesizes `--ref-text`; otherwise it uses a short smoke-test sentence. It refuses target-WPM outputs whose required tempo change exceeds `--max-stretch-ratio` (default 1.8), because extreme compression produces unusable audio. It writes WAVs plus `tmp/chatterbox_tts_tests/manifest.json`.
...
Two-stage upload workflow. Stage 1 is local and safe: `build_upload_package(episode_id)` copies `05_video/final.mp4`, captions, chosen thumbnail, the full `05_video/thumbnails/` candidate folder, Shorts, Short title-card thumbnails, and metadata into `06_metadata/youtube_upload_package/`, then writes `package_manifest.json` and `PACKAGE_SUMMARY.md` for review. Metadata includes a sparse story-derived tag list capped by `upload.max_tags` (default 3) and optional `publish_at` scheduling from `config.yaml`. If `upload.multilingual_subtitles_enabled` is true, package build translates long-form and Short `.srt` captions into the configured `upload.subtitle_languages` tracks (default English source + 10 high-reach languages: zh-Hans, es, hi, ar, fr, bn, pt, ru, ur, id) and records them as `caption_tracks`. Shorts packaging is relocation-tolerant: if `05_video/shorts/manifest.json` contains stale absolute paths after a project folder move, the packager falls back to the current `05_video/shorts/<filename>` or `short_NN.mp4`. Stage 2 is explicit: `upload_approved_package(episode_id, approve=True)` uploads the long-form video, thumbnail, every caption track, and each Short to YouTube, optionally adding videos to configured playlists. YouTube has playlists, not folders; set `upload.shorts_playlist_id` to the channel's Shorts playlist ID if Shorts should be grouped there. It refuses to run unless the CLI includes `--approve-youtube-upload`. Successful long-form uploads update the queue record's `youtube_video_id`; Shorts IDs go to `youtube_shorts_video_ids`. Thumbnail/caption/playlist failures are captured in `post_upload_warnings` and do not erase the successful video upload result.
...
ElevenLabs TTS adapter with the same `synthesize_script` interface as the other TTS backends. Activates when `cfg.tts.backend == "elevenlabs"`; `ELEVENLABS_API_KEY` must be in `.env`. Falls back to Kokoro on init failure (missing key) so flipping the backend is safe to try. Per-narrator voice_id pinning via `cfg.tts.elevenlabs.voice_id_map`.
...
- `--approve EP_ID` *(added Batch B 2026-05-26)* — clear any `needs_human` gate on the named episode so it can advance.
- `--auto-correct-s07 EP_ID` *(added Batch N.5 2026-05-31)* — apply every `suggested_rewrite` from `02_script/brand_safety_flags.json` to `script.txt`, write `brand_safety_autocorrect.json`, then clear only the S07 gate.
- `--auto-correct-s08 EP_ID` *(added Batch N.5 2026-05-31)* — apply any structured `original`/`replacement` suggestions found in S08 review artifacts to `beat_sheet.json`, write `beat_sheet_autocorrect.json`, then clear only the S08 gate.
- `--auto-approve-s09 EP_ID` *(added Batch N.5 2026-05-31)* — clear only the S09 visual brand-safety review gate.
...
- `--rerun-from EP_ID STAGE_ID` *(added 2026-05-28)* — reset the named stage AND every later stage back to `pending` so the orchestrator re-runs them. Use after a config-flag flip that invalidates an earlier stage's output: `asset_hunt.enabled false→true` (rerun-from S5), `sfx_library.enabled false→true` (S11), `tts.backend` changes (S10), narrator persona edit (S6), callout styling change (S12). Accepts `S5` / `s5` / `5`. Does NOT delete on-disk artifacts — those get overwritten when each stage runs.
...
- `--upload-youtube-package EP_ID --approve-youtube-upload [--youtube-privacy private|unlisted|public] [--youtube-publish-at RFC3339]` *(added Batch N.9 2026-06-01)* — upload the package to YouTube only after explicit approval. Defaults to `upload.default_privacy_status` from `config.yaml`; `--youtube-publish-at` or `upload.publish_at` schedules publishing and forces private-until-publish semantics. Writes `upload_results.json` and updates queue video IDs.
- `--backfill-youtube-captions EP_ID --approve-youtube-upload [--youtube-caption-target all|long|shorts|short_NN] [--youtube-caption-language LANG ...]` *(added Batch N.10 2026-06-02; rank fallback fixed same day)* — retry/upload packaged caption tracks against the existing uploaded YouTube video IDs without re-uploading videos. Reads `upload_results` from the package manifest, resolves Shorts IDs by explicit rank or upload order with queue fallback, skips caption languages already present on YouTube, stops cleanly on quota exhaustion, and records each run in `caption_backfill_runs`.
...
**Narrative spine + single-pass generation *(Batch N 2026-05-31; retry budget wired 2026-06-06; final beat-id hardening 2026-06-07; story-spine/fact-budget pass 2026-06-08; ending/quality guards 2026-06-08; single-pass default 2026-06-08)*:** S06 first runs `script_narrative_spine.txt` to lock the central question, timeline mode, protagonist desire, fatal misread, first big bet, hidden weakness, darkest moment, mid-roll tension, final image, `ending_mode`, `viewer_question`, and core/support/callout fact IDs. It writes `02_script/script_narrative_spine.json`, then filters the fact ledger down to the selected story facts. Default `production.script_generation_mode: single_pass` then calls `script_generate.txt` once per retry attempt to produce one continuous script using story stages inside the prompt rather than separate act calls. This preserves narrator rhythm and narrative flow better than isolated act chunks. The older blueprint/per-act route remains available with `script_generation_mode: act_by_act`; in that mode `script_blueprint.txt` is intentionally compact and S06 can fall back to a deterministic compact blueprint if JSON is malformed. Both modes use `production.max_script_generation_attempts`, length directives, forbidden-phrase scoring, local expand/condense repair, callout repair, beat renumbering, duplicate-beat cleanup, `02_script/script_story_quality_flags.json`, and `script_meta.json` recomputation. Preview mode still uses the compact full-script path.
...
Opening-hook support is checked inside the single-pass retry loop, not only after draft selection. If BEAT 1/2 invents a high-impact money/valuation/cover/frozen-operations claim (for example `$9B` when no such valuation exists in the ledger), the next retry receives a targeted hook-fact nudge.
Single-pass S06 attempts also score exact repeated sentences and unsupported document/source references (S-1, SEC filing, deposition, `appears verbatim`, etc.) during the retry loop. These objective story-quality flags are written to `script_story_quality_flags.json` and block S06 if no clean retry is produced.
S06 callout repair is decimal-safe: sentence scanning must not split `$4.5 million` / `$2.6 million` into `$4` / `$2` callouts. The story-quality gate also flags a small set of explicit unsupported phrases such as `real profit engine`, `smaller entity`, `fees are paid correctly`, or `state thought they were criminals` unless the ledger supports them. Broader tone and framing nuance belongs in S07, not the S06 retry loop.
S06 normalizes malformed beat markers like `## BE Beat 11 ##` before beat parsing. Before callout repair, it also applies a deterministic legal-neutralization pass for high-risk narrator phrasing (`sophisticated fraud`, `fraudulent model`, `valuation plummeted`, `investor confidence evaporated`, etc.) so S07 receives a cooler, more documentary draft.

`[CALLOUT: "$9 BILLION"]` markers are **mandatory** and config-driven (hardened Batch H 2026-05-28, tightened Batch J 2026-05-29, code-capped 2026-06-14). Batch J/N rules:
- HARD CAP: `callouts.max_total` across the entire script and `callouts.max_per_beat` inside each beat. S06 strips excess LLM-generated markers before S08, preferring numeric/date/money callouts and distributing retained markers across the episode. If the cap is still violated, S06 blocks.
- Word-only callouts are discouraged. Each CALLOUT should contain at least one digit OR a dollar sign OR a date abbreviation. "TIKTOK ERA", "BAD SUPER BOWL", "THE SHUTDOWN" are explicitly listed as wrong in the prompt.
- DEDUP: the same callout text cannot appear twice; the second occurrence quotes the number in prose without a marker.

**Callout repair/limiting *(Batch N.5 2026-05-31; limiter added 2026-06-14)*:** after generation, length repair, beat normalization, and forbidden-phrase substitutions, S06 counts `[CALLOUT: ...]` markers. If the writer under-delivers, `_ensure_callout_markers()` scans beat sentences for ledger-safe numeric/date patterns already present in the prose and inserts markers until `callouts.target_total` or `callouts.max_total`. If the writer over-delivers, `_limit_callout_markers()` strips the extras so malformed drafts with an overlay after every sentence cannot advance.
...
- The bounded retry budget is `production.max_script_generation_attempts` (default 100) so mandatory script generation keeps trying but cannot spin forever.
...
- `_enforce_hook_beat_intents()` — first 3 beats only rewrite flat `chart_abstraction` / `montage_panel` hooks to `document_or_headline`. Legal-document hooks stay legal-document hooks; they are no longer forced into `founder_portrait`, which caused unrelated founder/app imagery in bankruptcy openings.
...
- FLUX-prompt composition loop now uses per-beat `effective_visual_style` (loads the requested style YAML on demand) instead of the episode-level lock.
- `_apply_visual_continuity_plan()` *(Batch M 2026-05-30; prop cooldown Batch N.4 2026-05-31; episode prop palette 2026-06-20)* groups beats into deterministic scenes (`SCENE_NN`) and assigns a `story_world` (`legal_fraud`, `product_platform`, `name_confusion_bridge`, `business_context`, `closing_resolution`). Before scene planning, S08 builds `visual_prop_palette.json` by deterministically extracting story-specific props from `script.txt`, `script_narrative_spine.json`, `fact_ledger.json`, `asset_manifest.json`, and `character_profile.json`, then asking the writer LLM to refine those candidates through `prompts/visual_prop_palette.txt`. Generic continuity symbols such as cracked phones, red/green folders, leaf logos, and generic evidence folders are filtered unless truly story-central. S08 rewrites each FLUX fallback as a movie-shot prompt with narrative purpose, selected continuity props, scene title, and beat-specific `must_not_show` constraints. S08 keeps the full `recurring_props` list for auditability, but the actual prompt uses `active_recurring_props` only. `config.yaml > visual_continuity.props_per_beat` controls how many props enter one prompt, `prop_cooldown_beats` avoids props from recent slides, and `avoid_recently_used_props` adds an explicit prompt clause. Output includes top-level `visual_prop_palette` and `visual_continuity_plan` plus per-beat `episode_prop_palette`, `scene_id`, `scene_title`, `story_world`, `recurring_props`, `active_recurring_props`, `avoid_recent_props`, `visual_must_not`, `movie_shot_prompt`, and `visual_prompt_version`.
- Iconic-assets preamble (`00_research/iconic_assets.json`, generated by S01) is now gated *(Batch M 2026-05-30)* instead of prepended to every FLUX prompt. Product cues are allowed only on product/founder/product-reveal beats; legal/fraud and document/court beats explicitly suppress phones, app UI, generic leaf logos, and founder-glamour imagery.
- PD references are now high-confidence only for FLUX grounding *(Batch M 2026-05-30)*: reference threshold is floored at 0.52 and `_asset_reference_allowed()` rejects weak/off-topic assets such as vinyl records, music-label logos, sheet music, or decorative "vine" imagery unless the beat/story-world genuinely supports them.
- `_dedupe_beat_rows()` *(Batch N.1 2026-05-31)* drops duplicate LLM-emitted `beat_id` rows before callout parsing, scene planning, and FLUX prompt generation. The script remains the authoritative source of beat IDs; duplicate beat-sheet rows would otherwise create extra video panels with no matching TTS timing.
...
1. Builds the prompt from the visual-style prefix/suffix + S08's storyboard/movie-shot prompt + tightly gated character/iconic cues + high-confidence PD reference caption, when applicable.
...
Builds per-beat clips, prepends the opening title card, appends the configured music-only tail / optional closing material, muxes against `04_audio/final_mix.wav`, and writes ASR-aligned captions when available. The opening title card starts from `03_assets/flux/title.png` when present, then composites the real company logo from `03_assets/title_logo.json` / `03_assets/pd/company_logo.png` with Pillow. `production.title_card_logo_corner` controls the logo corner (default top-right); the yellow Pillow title is forced to the diagonal opposite corner, shrinks to fit the company name on one row across the safe frame width, `production.title_card_logo_width_pct` controls the logo cap with frame-edge guards, and `05_video/title_card_meta.json` records the resolved corners. S12 trims transparent/near-white canvas around the logo before fitting so wordmarks do not render tiny because of empty source-image padding, but preserves white-on-transparent wordmarks instead of mistaking them for removable white canvas.

### S10 — TTS Render (`s10_kokoro_render.py`)
Per-narrator voice render through `make_tts()`. Default backend is Chatterbox; Kokoro and ElevenLabs remain selectable via `tts.backend`. Pronunciation overrides at `pipeline/lexicon/pronunciation_overrides.yaml` (Bezos, Theranos, Zuckerberg, Wirecard, EBITDA, IPO, etc.). Output: per-beat WAV chunks + `voice_full.wav`.

**Configured WPM enforcement *(Batch N.3 2026-05-31; Chatterbox default 2026-06-04; prompt cadence wired 2026-06-08)*:** TTS backends can drift away from the documentary cadence assumed by `production.wpm_effective`. When `production.enforce_tts_wpm_effective: true`, S10 writes `voice_full.raw_tts.wav`, computes target duration from the cleaned script word count and `wpm_effective` (currently 150), then uses ffmpeg `atempo` chaining via `ffmpeg_builder.time_stretch_audio()` to create `voice_full.wav` at the configured cadence. S06/S07 hook-cadence word windows and S08 fallback beat-duration estimates also read the same config value. `voice_timing.json` records `source_word_count`, `wpm_effective`, and `wpm_enforced`.
...
**Beat-id timing hardening *(2026-06-07)*:** S10 now records `voice_timing.json` with the literal `## BEAT N ##` marker IDs from `script.txt` instead of inventing a fresh sequential counter. Non-contiguous markers are logged as warnings. S06 should prevent those in normal operation, but this keeps TTS timing aligned with S08's beat sheet if a malformed legacy script reaches S10.
...
- **Voice padding for title + closing/tail cards *(Batch J 2026-05-29; closing toggle Batch M.3 2026-05-30; conclusion tail Batch N.3 2026-05-31)*:** before any mixing, `voice_full.wav` is padded with `production.opening_title_card_seconds` of silence at the head and `production.conclusion_music_tail_seconds` at the tail. When `production.closing_card_enabled` is true, S11 also appends `production.closing_card_seconds + 1.0s` for the credits card. The padded file `voice_padded.wav` becomes the mix input. Reason: S12 prepends a title card and can append a music-only final-panel hold / closing card around the per-beat clips, and S12's `concat_clips` uses `-shortest`.
...
- **Phase 3: Shorts** — `pipeline.shorts.generate_teaser_script()` asks the writer LLM for separate compressed viral teasers from the full script and beat table (`prompts/shorts_teaser_script.txt`). S13 renders separate standalone TTS tracks and normalizes them to `packaging.shorts_tts_wpm` (default 200), then builds 2 vertical Shorts from reused beat images with rapid cuts, fast Ken Burns motion, slide-left xfade transitions, and no music/SFX. English captions are chunked into short readable cues, optionally hard-burned as Pillow overlays, and saved as `.srt` for upload packaging. Output: `05_video/shorts/short_NN.mp4` + `short_NN.srt` + `teaser_script_NN.txt` + `manifest.json`.

### S12 — Video Assembly (`s12_video_assembly.py`)
- Title card: FLUX-rendered `title.png` background, episode title composited via Pillow (yellow body, black stroke, random-corner placement, deterministic by episode id). Batch N.3 makes the title clip a static `hold_still` one-second render with no fade or Ken Burns motion. The default title font size is `production.title_card_font_size_pct: 0.26` (about 280 px at 1080p) for phone legibility.
- Per-beat clip: Ken Burns motion over the beat's image, fade in/out (`fade_in_seconds`/`fade_out_seconds`).
- **Beat/timing alignment *(Batch N.1 2026-05-31)*:** S12 aligns beat-sheet rows to `voice_timing.json` order and keeps one row per voiced beat. Duplicate beat-sheet IDs and beat-sheet rows missing from the TTS timeline are skipped with warnings. This fixes EP004's 9.5s pre-mux drift, where S08 emitted two `BEAT_48` rows but S10 produced only one `BEAT_48` audio segment.
- **Conclusion music tail *(Batch N.3 2026-05-31)*:** when `production.conclusion_music_tail_seconds > 0`, S12 appends `zz_conclusion_tail.mp4`, a static hold on the final beat image for that duration. S11 pads the voice with matching silence so the music bed continues alone after narration ends.
- **Like/subscribe outro clip *(Batch N.6 2026-05-31; default off 2026-06-03)*:** when `production.append_like_subscribe_clip: true`, S12 appends `production.like_subscribe_clip_path` (default `assets/video/like_and_subscribe.mp4`) after the conclusion tail / closing card. Default is now `false`, so `final.mp4` ends after the configured conclusion tail unless the operator explicitly re-enables the outro. If the configured clip is video-only, S12 pads the mux audio with matching trailing silence so ffmpeg's `-shortest` does not trim the outro away.
- **Callout overlays *(added Batch C 2026-05-27, cache fixes Batch J/K 2026-05-29, no-cache + loud diagnostics Batch L 2026-05-29, ASR timing 2026-06-05)*:** when a beat has a non-empty `callouts` list (populated by S08 from inline `[CALLOUT: "..."]` markers in the script), S12 uses `04_audio/voice_asr_segments.json` when available to place the callout near the actual spoken sentence; otherwise it falls back to the word-ratio estimate inside the beat. `ffmpeg_builder.composite_callouts_onto_clip()` renders each callout as a Pillow text PNG (yellow + black stroke, same styling as the title card), then overlays it on the clip with timed visibility + fade in/out. Max callouts per beat configurable via `callouts.max_per_beat` (default 1). **No-cache for `{beat_id}_callout.mp4` (Batch L 2026-05-29)**: every `*_callout.mp4` is purged at S12 entry and re-composited from the raw `{beat_id}.mp4`. The cache savings on a ~1-2s overlay render were dwarfed by the cache-staleness bugs they caused — final3.mp4 and final4.mp4 both shipped without overlays because the Batch J cache-key fix and Batch K mtime invalidator only handled "upstream artifact changed", not "S12 / composite code changed since the cached clip was made". The raw `{beat_id}.mp4` Ken Burns clips stay cached (expensive supersample renders) with the Batch K `_any_newer(inputs, cached)` mtime invalidator keyed on `beat_sheet.json`. **Loud diagnostics (Batch L 2026-05-29)**: every `return False` path in `composite_callouts_onto_clip` now carries a specific `logger.warning` (empty list / Pillow font fallthrough to bitmap default / all-empty text / ffmpeg returned but output 0-byte / unhandled exception). On success the function logs `composite_callouts: BEAT_NN_callout.mp4 ← N overlays via BEAT_NN.mp4 (font=<path> @ <px>)` so the operator can grep the daily log to confirm the overlay path ran.
- Concat all clips, mux voice + music + SFX, and write SRT + VTT. Long-form captions prefer ASR timings from `voice_full.wav`; `voice_timing.json` estimates are fallback only.
...
| `script_narrative_spine.txt` *(added 2026-06-08)* | S6 primary | `{incident_name}, {fact_ledger_json}, {target_words}, …` | Locks central question, timeline mode, protagonist desire, fatal misread, mid-roll tension, final image, ending mode/viewer question, and selected fact IDs before prose. |
| `script_generate.txt` | S6 primary / preview | `{incident_name}, {narrative_spine_json}, {fact_ledger_json}, {target_words}, ...` | Compact single-pass story-arc prompt using configured long-form WPM; writes one continuous script with beat markers. |
| `script_blueprint.txt` *(added Batch N 2026-05-31; causal spine update 2026-06-08; compact output 2026-06-08)* | S6 primary | `{incident_name}, {narrative_spine_json}, {fact_ledger_json}, {act_specs_json}, …` | Builds a compact causal act/fact blueprint before prose generation; beat-level outlining is deferred to the act writer. |
| `script_act_generate.txt` *(added Batch N 2026-05-31; causal spine update 2026-06-08)* | S6 primary | `{act_id}, {narrative_spine_json}, {act_blueprint_json}, {beat_start}, …` | Writes one act at a time using the relevant causal blueprint slice and filtered fact ledger. |
| `script_critique.txt` | S7 | `{script}` | Retention, chronology, story-spine, repetition, and voice audit. |
...
| `iconic_assets.txt` *(added Batch F 2026-05-28)* | S01 tail | `{company_name}, {founder}, {year_anchor}, {story_kind}, {hero}, {conflict}, {one_line_pitch}` | Derives the company's iconic visual cues (mascot, logo shape, signature products, era markers, recurring protagonist appearance). Output to `00_research/iconic_assets.json`; S08 now gates these cues by story-world instead of prepending them to every FLUX prompt. |
...
| `V3.yaml` | **Sunlit Editorial Comic**. Warm natural daylight, clean confident ink, premium editorial composition, human-centered optimistic brand-story visuals. For origin / disruption / pivot / underdog comeback episodes. |
...
- `pipeline/lexicon/pronunciation_overrides.yaml` — pronunciation map applied before S10 sends text to the active TTS backend. Format: `"Word": "PRO-nun-see-AY-shun"`.
...
| `opening_title_card_seconds` / `conclusion_music_tail_seconds` / `closing_card_enabled` / `closing_card_seconds` changed | `S11` (re-pads voice, re-mixes, re-renders) |
| Visual style YAML edited (V1.yaml / V2.yaml / V3.yaml) | `S9` |
...
- **Chatterbox is now the default TTS backend** — `tts.backend: chatterbox` uses the oMLX `/v1/audio/speech` endpoint with `chatterbox-turbo-4bit`. Long form enforces `production.wpm_effective` (currently 150); Shorts enforce `packaging.shorts_tts_wpm: 200`. Kokoro remains available with `tts.backend: kokoro`.
...
- **N8 Felix Female Voice added** — `config.yaml > narrators` now enables N5/N6/N7/N8. N8 uses Felix's dry N5 persona instructions via a YAML anchor in `pipeline/style_profiles/narrators.yaml`, while the active TTS backend maps it to a female voice label or reference.
...
- **S12 like/subscribe outro is optional and off by default** — `production.append_like_subscribe_clip` can add `assets/video/like_and_subscribe.mp4` to the end of final videos, but the default is `false`. S12 pads mux audio by the clip duration when enabled so the video-only mock outro is not trimmed.
- **Opening title text is larger for mobile** — `production.title_card_font_size_pct` is `0.26`, roughly double the earlier title size.
- **Shorts avoid Pillow callout beats** — `packaging.shorts_exclude_callout_beats` removes callout beats from the candidate set and rejects windows that overlap callout timing intervals. The cutter now maps source audio explicitly; Whisper remains subtitle-only.
...
- **S06 now repairs missing callouts** — `callouts.target_total` defaults to 5, and S06 backfills missing `[CALLOUT: ...]` markers from numeric/date sentences already present in the script before S08 parses the beat sheet.
- **New gate automation CLIs** — `--auto-correct-s07`, `--auto-correct-s08`, and `--auto-approve-s09` provide stage-specific gate handling without approving unrelated blockers.
- **S10 now enforces configured narration cadence** — `production.enforce_tts_wpm_effective` time-stretches `voice_full.wav` to the duration implied by script word count and `production.wpm_effective`, fixing TTS renders that come out faster or slower than the planned documentary pace.
...
- **S08 now cools down recurring props** — `visual_continuity.props_per_beat` and `prop_cooldown_beats` keep signature props recurring across the episode without repeating the same phone/folder symbols on every adjacent slide.
...
- **Weak FLUX reference assets are suppressed** — S08 floors the PD-reference threshold at 0.52 and rejects off-topic references like vinyl records, music-label logos, sheet music, and decorative vine art unless they genuinely match the beat.
- **Iconic company cues are no longer global** — S08 only injects product/iconic cues where the story-world and visual intent justify them. Legal/fraud beats now actively ban phones, app UI, generic leaf logos, startup pitch rooms, and founder-glamour portraits.
- **S09 re-renders when storyboard prompts change** — `image_qa.prompt_hash` and `visual_prompt_version` prevent old passing images from being reused after a regenerated S08 visual plan.
