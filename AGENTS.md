...
Writer LLM with `script_narrative_spine.txt` → `script_blueprint.txt` → `script_act_generate.txt`; `script_generate.txt` is now only a compact fallback prompt. **Compact seven-act retention template at 160 wpm** *(retuned 2026-06-05 for a 9-11 minute target with one mid-roll tension landing; story-spine pass added 2026-06-08):*
...
| `script_generate.txt` | S6 fallback / preview | `{incident_name}, {narrative_spine_json}, {fact_ledger_json}, {target_words}, ...` | Compact full-script fallback at 160 wpm; trimmed from ~3.9k words to a small emergency path for Qwen stability. |
...
- **Chatterbox is now the default TTS backend** — `tts.backend: chatterbox` uses the oMLX `/v1/audio/speech` endpoint with `chatterbox-turbo-4bit`. Long form enforces `production.wpm_effective: 160`; Shorts enforce `packaging.shorts_tts_wpm: 200`. Kokoro remains available with `tts.backend: kokoro`.
