...
- `_apply_visual_continuity_plan()` *(Batch M 2026-05-30; prop cooldown Batch N.4 2026-05-31)* groups beats into deterministic scenes (`SCENE_NN`) and assigns a `story_world` (`legal_fraud`, `product_platform`, `name_confusion_bridge`, `business_context`, `closing_resolution`). It rewrites each FLUX fallback as a movie-shot prompt with narrative purpose, selected continuity props, scene title, and beat-specific `must_not_show` constraints. S08 keeps the full `recurring_props` list for auditability, but the actual prompt uses `active_recurring_props` only. `config.yaml > visual_continuity.props_per_beat` controls how many props enter one prompt, `prop_cooldown_beats` avoids props from recent slides, and `avoid_recently_used_props` adds an explicit prompt clause. Output includes top-level `visual_continuity_plan` plus per-beat `scene_id`, `scene_title`, `story_world`, `recurring_props`, `active_recurring_props`, `avoid_recent_props`, `visual_must_not`, `movie_shot_prompt`, and `visual_prompt_version`.
