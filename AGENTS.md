...
- **Phase 3: Shorts** — `pipeline.shorts.generate_teaser_script()` asks the writer LLM for one compressed viral teaser from the full script and beat table (`prompts/shorts_teaser_script.txt`). S13 renders one standalone TTS track using Kokoro speed directly by default, then builds 3 vertical Shorts from reused beat images with rapid cuts, fast Ken Burns motion, slide-left xfade transitions, and no music/SFX. English captions are chunked into short readable cues, optionally hard-burned as Pillow overlays, and saved as `.srt` for upload packaging. Output: `05_video/shorts/short_NN.mp4` + `short_NN.srt` + `manifest.json`.
...
- **Like/subscribe outro clip *(Batch N.6 2026-05-31)*:** when `production.append_like_subscribe_clip: true`, S12 appends `production.like_subscribe_clip_path` (default `assets/video/like_and_subscribe.mp4`) after the conclusion tail / closing card. If the configured clip is video-only, S12 pads the mux audio with matching trailing silence so ffmpeg's `-shortest` does not trim the outro away.
...
- **S12 appends the configured like/subscribe outro** — `production.append_like_subscribe_clip` adds `assets/video/like_and_subscribe.mp4` to the end of every final video. S12 pads mux audio by the clip duration when needed so the video-only mock outro is not trimmed.
