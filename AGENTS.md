...
Writer LLM with `script_narrative_spine.txt` → `script_blueprint.txt` → `script_act_generate.txt`; `script_generate.txt` is now only a compact fallback prompt. **Compact seven-act retention template at configured long-form WPM, currently 150** *(retuned 2026-06-05 for a 9-11 minute target with one mid-roll tension landing; story-spine pass added 2026-06-08; 150wpm documentary pace wired 2026-06-08):*
...
**Narrative spine + act-by-act generation *(Batch N 2026-05-31; retry budget wired 2026-06-06; smaller staged budgets 2026-06-06; final beat-id hardening 2026-06-07; story-spine/fact-budget pass 2026-06-08; ending/quality guards 2026-06-08; compact blueprint fallback 2026-06-08)*:** when `production.script_act_by_act_enabled: true`, S06 first runs `script_narrative_spine.txt` to lock the central question, timeline mode, protagonist desire, fatal misread, first big bet, hidden weakness, darkest moment, mid-roll tension, final image, `ending_mode`, `viewer_question`, and core/support/callout fact IDs. It writes `02_script/script_narrative_spine.json`, then filters the fact ledger down to the selected story facts before blueprint/prose generation. `script_blueprint.txt` is intentionally compact: it assigns act budgets, causal turns, fact IDs, must-include events, recurring props, and callout opportunities, but it does **not** ask the model to outline every beat. If the blueprint JSON is still malformed/truncated, S06 uses a deterministic compact fallback blueprint rather than burning the full 100-attempt script retry budget on the same parse failure. `script_act_generate.txt` writes one act at a time with the relevant blueprint slice, previous-act tail, narrator persona, filtered fact ledger, and beat range. The staged path uses `production.max_script_generation_attempts`, adding stronger length directives between attempts and keeping the closest clean draft if all attempts miss. Act budgets are intentionally scaled by `production.script_staged_word_budget_multiplier` (default `0.82`) because the act writer naturally overshoots; for a 1550-word target it feeds ~1271 words of act ceilings so the final draft lands inside the 1400-1650 gate. The output is renumbered, cleaned, repaired for near-miss length issues, and only falls back to the compact `script_generate.txt` full-script retry loop if staged generation errors on every attempt or returns no draft. After every late mutation, including substitutions, short-beat merge, and callout repair, S06 renumbers beats again, removes adjacent/nearby duplicate beat bodies, writes `02_script/script_story_quality_flags.json`, and recomputes `script_meta.json` so S08 and S10 cannot disagree on beat IDs. Preview mode still uses the compact full-script path.
...
| `script_generate.txt` | S6 fallback / preview | `{incident_name}, {narrative_spine_json}, {fact_ledger_json}, {target_words}, ...` | Compact full-script fallback using configured long-form WPM; trimmed from ~3.9k words to a small emergency path for Qwen stability. |
