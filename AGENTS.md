...
- `--backfill-youtube-captions EP_ID --approve-youtube-upload [--youtube-caption-target all|long|shorts|short_NN] [--youtube-caption-language LANG ...]` *(added Batch N.10 2026-06-02)* — retry/upload packaged caption tracks against the existing uploaded YouTube video IDs without re-uploading videos. Reads `upload_results` from the package manifest, skips caption languages already present on YouTube, stops cleanly on quota exhaustion, and records each run in `caption_backfill_runs`.
