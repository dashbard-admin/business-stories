...
**Blueprint + act-by-act generation *(Batch N 2026-05-31; retry budget wired 2026-06-06)*:** when `production.script_act_by_act_enabled: true`, S06 no longer asks the writer for one huge script first. It runs `script_blueprint.txt` to assign act budgets, fact IDs, hook turns, beat goals, recurring props, and callout opportunities, writes `02_script/script_blueprint.json`, then calls `script_act_generate.txt` once per act with the relevant act blueprint, previous-act tail, narrator persona, fact ledger, and beat range. The staged path now uses `production.max_script_generation_attempts`, adding stronger length directives between attempts and keeping the closest clean draft if all attempts miss. The output is renumbered, cleaned, repaired for near-miss length issues, and only falls back to the older full-script retry loop if staged generation errors on every attempt or returns no draft. Preview mode still uses the older compact full-script path.
