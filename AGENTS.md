...
Two-stage upload workflow. Stage 1 is local and safe: `build_upload_package(episode_id)` copies `05_video/final.mp4`, captions, chosen thumbnail, Shorts, and metadata into `06_metadata/youtube_upload_package/`, then writes `package_manifest.json` and `PACKAGE_SUMMARY.md` for review. Metadata includes a sparse story-derived tag list capped by `upload.max_tags` (default 3) and optional `publish_at` scheduling from `config.yaml`. Stage 2 is explicit: `upload_approved_package(episode_id, approve=True)` uploads the long-form video, thumbnail, caption file, and each Short to YouTube, optionally adding videos to configured playlists. YouTube has playlists, not folders; set `upload.shorts_playlist_id` to the channel's Shorts playlist ID if Shorts should be grouped there. It refuses to run unless the CLI includes `--approve-youtube-upload`. Successful long-form uploads update the queue record's `youtube_video_id`; Shorts IDs go to `youtube_shorts_video_ids`.
...
- **YouTube package/upload workflow added** — `youtube_upload.py` creates a self-contained upload package with long-form video, captions, thumbnail, Shorts, metadata, sparse story-derived tags, optional `publish_at`, and summaries. Upload requires `--upload-youtube-package EP_ID --approve-youtube-upload`, uses the expanded OAuth token, uploads long-form + Shorts, sets thumbnail/captions/playlists where configured, writes `upload_results.json`, and binds uploaded IDs back to the queue.
