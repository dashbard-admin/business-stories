...
Builds 3 fast standalone Shorts via `prompts/shorts_teaser_script.txt`. S13 passes the full script + beat table to the writer LLM, gets one compressed 30-second viral teaser script with a hook in the first ~5 seconds, renders one fast TTS track (`packaging.shorts_tts_wpm`, default 230; `shorts_tts_speed`, default 1.85), then assembles each Short from reused beat images with rapid 3-5 second cuts (`shorts_seconds_per_image`, default 3.75). Shorts have no music or SFX. English teaser captions are generated from the teaser script and can be hard-burned via `shorts_burn_subtitles`; the `.srt` files are also emitted for YouTube subtitle upload. The old window-picking/cutting helpers remain in the module for fallback/comparison, but the active S13 path is teaser-first. Output: `05_video/shorts/short_NN.mp4`, `short_NN.srt`, `teaser_script.txt`, and `manifest.json`.
...
- **Phase 3: Shorts** — `pipeline.shorts.generate_teaser_script()` asks the writer LLM for one compressed viral teaser from the full script and beat table (`prompts/shorts_teaser_script.txt`). S13 renders one fast standalone TTS track at `packaging.shorts_tts_wpm` (default 230 WPM), then builds 3 vertical Shorts from reused beat images with `packaging.shorts_seconds_per_image` rapid cuts and no music/SFX. English captions are generated from the teaser script, optionally hard-burned, and saved as `.srt` for upload packaging. Output: `05_video/shorts/short_NN.mp4` + `short_NN.srt` + `manifest.json`.
...
*This file last updated: 2026-06-02 — fast teaser Shorts and multilingual subtitles.*
